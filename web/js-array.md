...
| `forEach` | 간결하고 가독성이 높음. callback 함수를 이용하여 각 요소를 조작하기 용이 | 사용 불가 | 사용 권장 |
...
| `some` | 배열의 요소 중 적어도 하나라도 콜백함수를 통과하면 true를 반환하며, 즉시 배열 순회 중지. 모두 통과하지 못하면 false를 반환 |
| `every` | 배열의 모든 요소가 콜백함수를 통과하면 true를 반환. 하나라도 통과하지 못하면 즉시 false를 반환하고 배열 순회 중지 |
