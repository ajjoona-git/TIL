...
[![GitHub Repository](https://img.shields.io/badge/GitHub-ajjoona--git/Vibe--Chatbot-blue?style=flat&logo=github)](https://github.com/ajjoona-git/Vibe-Chatbot)
...
### 오류 해결
파일에 API Key가 포함되어 있을 경우 git push가 안 된다.
API Key는 노출되면 안 되기 때문!

1) `.env` 파일에 API Key를 저장하는 변수 입력하고
2) `.gitignore`에 `.env` 파일 추가
3) 파일 내부 API Key 부분을 변수 이름으로 변경한다.
* 단, 이미 API key를 포함한 파일을 commit한 적이 있다면 `.git` 폴더를 삭제한 후 `git init`부터 다시 진행해야 한다.
