...
- MCP Host: 하나 또는 여러 개의 MCP 클라이언트를 조정하고 관리하는 AI 애플리케이션. Claude Desktop, IDE, AI도구와 같이 사용자가 직접 상호작용하는 프로그램
- MCP Client: MCP 서버와의 연결을 유지하며 MCP 호스트가 사용할 수 있도록 MCP 서버로부터 컨텍스트를 가져오는 구성요소. 호스트와 서버 간 1:1 연결을 유지하는 프로토콜 클라이언트
- MCP Server: MCP 클라이언트에게 컨텍스트를 제공하는 프로그램. 특정 기능을 표준화된 프로토콜을 통해 노출하는 경량 프로그램
